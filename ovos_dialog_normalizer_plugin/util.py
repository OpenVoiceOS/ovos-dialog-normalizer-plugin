...
def _normalize_dates_and_times(text: str, full_lang: str, date_format: str = "DMY") -> str:
    ...
    if lang_code == "en":
        # Only expand am/pm when they are attached to a clock number,
        # so unrelated words such as "am" in "I am" are left untouched.
        text = re.sub(r"(?i)\b(\d{1,2})\s*am\b", r"\1 A M", text)
        text = re.sub(r"(?i)\b(\d{1,2})\s*pm\b", r"\1 P M", text)
